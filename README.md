...
## Fix de instalación en Cloudflare Builds

Esta versión no incluye `package-lock.json` para evitar que Cloudflare ejecute `npm clean-install` / `npm ci` con un lockfile generado en otro entorno. Cloudflare debe instalar con `npm install` desde `package.json`.

Si el build anterior falló instalando dependencias, en Cloudflare usa **Retry deployment → Clear cache and retry** después de subir esta versión.

Configuración recomendada en Cloudflare Workers Builds:

- Build / deploy command: `npx wrangler deploy`
- Install command: dejar por defecto, o usar `npm install --no-package-lock --progress=false`
